...
# ---- FILE UPLOAD ----
uploaded_file_1 = st.file_uploader("Upload your 1st .xlsx file here", type=["xlsx"])
uploaded_file_2 = st.file_uploader("Upload your 2nd .xlsx file here", type=["xlsx"])
uploaded_file_3 = st.file_uploader("Upload your 3rd .xlsx file here", type=["xlsx"])

# ---- DATA PROCESSING FUNCTION ----
@st.cache_data
def get_crime_data(file1, file2, file3):
    ...
    data_file_1 = pd.read_excel(file1, sheet_name="Sheet1")
    data_file_2 = pd.read_excel(file2, sheet_name="Sheet1")
    data_file_3 = pd.read_excel(file3, sheet_name="Sheet1")

    # Standardize column names for consistency
    data_file_1.columns = ["Region", "Crime_Type", "Year", "Incident_Count"]
    data_file_2.columns = ["Region", "Crime_Type", "Year", "Incident_Count"]
    data_file_3.columns = ["Region", "Crime_Type", "Year", "Incident_Count"]

    # Combine datasets into a single DataFrame
    combined_df = pd.concat([data_file_1, data_file_2, data_file_3], ignore_index=True)
    
    return combined_df

# Load data
df = get_crime_data(uploaded_file_1, uploaded_file_2, uploaded_file_3)
